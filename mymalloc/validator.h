...
static int add_range(const malloc_impl_t *impl, range_t **ranges, char *lo,
  ...
  if (lo < (char *)impl->heap_lo() || hi > (char *)impl->heap_hi()) {
    malloc_error(tracenum, opnum, "OUT OF RANGE!");
    return 0;
  }
  // The payload must not overlap any other payloads
  // TODO(project3): YOUR CODE HERE
  range_t *temp = ranges[0];
  while (temp) {
    if (!(hi < temp->lo || lo > temp->hi)) {
      malloc_error(tracenum, opnum, "OVERLAP!");
      return 0;
    }
    temp = temp->next;
  }
  ...
  // TODO(project3):  YOUR CODE HERE
  range_t *correct = (range_t *)malloc(sizeof(range_t));
  correct->hi = hi;
  correct->lo = lo;
  correct->next = ranges[0];
  ranges[0] = correct;
...
static void remove_range(range_t **ranges, char *lo) {
  range_t *p = NULL;
  range_t *prevpp = ranges[0];
  p = ranges[0];
  while (p != NULL && p->lo != lo) {
    prevpp = p;
    p = p->next;
  }
  if (p == NULL) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case REALLOC: // realloc
      ...
      for (int j = 0; j < oldsize; j++) {
        if (newp[j] != (char)index) {
          malloc_error(tracenum, i, "realloc failed to preserve data");
          return 0;
        }
      }
      memset(newp, index, size);
      // Remember region
